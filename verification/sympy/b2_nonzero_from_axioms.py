#!/usr/bin/env python3
"""
b₂ ≠ 0 from AXM_0109 (Crystal Existence)

KEY FINDING: The quartic coefficient b₂ in the tilt potential on Herm(n_d)
CANNOT be zero if crystals have a definite structure (AXM_0109).
b₂ = 0 gives degenerate minima (energy independent of eigenvalue partition).
Therefore AXM_0109 forces b₂ ≠ 0.

The sign b₂ < 0 (needed for SU(3)×U(1)) follows if crystallization creates
maximal symmetry breaking, but this step uses [A-PHYSICAL] interpretation.

Formula: F(k) = -k a² / (4(b₁ k + b₂))  for k nonzero eigenvalues
When b₂ = 0: F(k) = -a²/(4 b₁) for ALL k → degenerate → no definite crystal

Status: DERIVATION (b₂ ≠ 0), CONJECTURE (sign b₂ < 0)
...
#   F = -a Tr(ε²) + b₁ (Tr(ε²))² + b₂ Tr(ε⁴)
# with a > 0, b₁ > 0.
#
# A k-partition has k eigenvalues = λ, (n_d - k) = 0.
# Then: Tr(ε²) = k λ², Tr(ε⁴) = k λ⁴
...
# ===================================================================
# Substitute λ² = a / (2(b₁ k + b₂)) into F:
F_eq = F_general.subs(lam**2, lam_sq_eq).subs(lam**4, lam_sq_eq**2)
F_eq = simplify(F_eq)
print(f"\nF(k) at equilibrium = {F_eq}")

# Should be -k a² / (4(b₁ k + b₂))
F_expected = -k * a**2 / (4 * (b1 * k + b2))
print(f"Expected: {F_expected}")
print(f"Match: {simplify(F_eq - F_expected) == 0}")

# ===================================================================
# PART 3: b₂ = 0 → degenerate (key result)
# ===================================================================
print("\n" + "="*70)
print("PART 3: b₂ = 0 GIVES DEGENERATE MINIMA")
print("="*70)

F_b2_zero = F_expected.subs(b2, 0)
F_b2_zero = simplify(F_b2_zero)
print(f"F(k) when b₂ = 0: {F_b2_zero}")
# Should be -a²/(4 b₁), independent of k

# Check k-independence explicitly:
dF_dk_b2zero = diff(F_b2_zero, k)
print(f"∂F/∂k when b₂ = 0: {simplify(dF_dk_b2zero)}")
is_degenerate = simplify(dF_dk_b2zero) == 0
print(f"Is degenerate (∂F/∂k = 0)? {is_degenerate}")

# ===================================================================
# PART 4: b₂ < 0 → k=1 is global minimum
# ===================================================================
print("\n" + "="*70)
print("PART 4: b₂ < 0 → k=1 IS GLOBAL MINIMUM")
print("="*70)

# F(k) = -k a² / (4(b₁ k + b₂))
# Compare F(1) and F(k) for k > 1:
# F(1) - F(k) = (a²/4) [k/(b₁ k + b₂) - 1/(b₁ + b₂)]
#             = (a²/4) [k(b₁ + b₂) - (b₁ k + b₂)] / [(b₁ k + b₂)(b₁ + b₂)]
#             = (a²/4) [b₂(k - 1)] / [(b₁ k + b₂)(b₁ + b₂)]
...
print(f"  (k - 1) > 0")
print(f"  a² > 0")
print(f"  Denominators positive (assuming b₁ k + b₂ > 0)")
print(f"  Sign of F(1) - F(k) = sign of b₂")
print(f"  If b₂ < 0: F(1) - F(k) < 0 → F(1) < F(k) → k=1 IS global min ✓")
print(f"  If b₂ > 0: F(1) - F(k) > 0 → F(1) > F(k) → k=1 is NOT min ✗")
...
for b2_val in [Rational(-1, 20), 0, Rational(1, 20)]:
    ...
    for k_val in range(1, 5):
        ...
        print(f"    k={k_val}: λ² = {float(lam_sq_val):.6f}, "
              f"F = {float(F_val):.8f}")
...
# ===================================================================
print("\n" + "="*70)
print("PART 6: AXIOM ARGUMENT FOR b₂ ≠ 0")
print("="*70)

print("""
THEOREM: b₂ ≠ 0 from AXM_0109 (Crystal Existence)

Proof sketch:
  [AXIOM] AXM_0109: A crystalline configuration ε* exists with specific
  properties (definite eigenvalue partition determining gauge structure).

  [THEOREM] For b₂ = 0, F(k) = -a²/(4b₁) for ALL k = 1,...,n_d.
...
CONJECTURE: b₂ < 0 from crystallization interpretation

  [A-PHYSICAL] Crystallization creates maximal structure (maximal
  symmetry breaking). The k=1 partition breaks U(n_d) → U(n_d-1)×U(1)
  maximally. This requires b₂ < 0.

  Alternative argument:
  [A-PHYSICAL] The k=n_d partition (b₂ > 0) preserves U(n_d) symmetry
...
For eigenvalue partition with k nonzero equal eigenvalues:
  ...
  b₂ < 0 selects k=1 (maximum concentration) ✓
  b₂ > 0 selects k=4 (minimum concentration) ✗
""")
...
# ===================================================================
print("\n" + "="*70)
print("PART 8: STABILITY AT k=1 MINIMUM (b₂ < 0)")
print("="*70)

# For n_d = 4, k=1: eigenvalues (λ, 0, 0, 0)
...
print(f"\n∂²F/∂l₂² at (λ*,0,0,0) = {d2F_dl2dl2}")

# Substitute λ*²:
if lam_star_sq:
    d2F_sub = d2F_dl2dl2.subs(lam_star**2, lam_star_sq[0])
    d2F_sub = simplify(d2F_sub)
    print(f"After substituting λ*²: {d2F_sub}")
    print(f"For stability, need ∂²F/∂l₂² > 0")
    print(f"This is: -2a + 4 b1 λ*² = -2a + 4b1 * a/(2(b1-b2))")
    manual = -2*a_s + 4*b1_s * a_s / (2*(b1_s + b2_s))
    manual = simplify(manual)
    print(f"= {manual}")
    print(f"= -2a b2 / (b1 + b2)")
    print(f"Since b2_s > 0 (meaning actual b₂ < 0) and b1 > b2_s: this is NEGATIVE")
    print(f"Wait — let me recheck...")

# Let me redo with explicit negative b2
print("\n--- Redo with explicit b₂ < 0 ---")
b2_neg = symbols('b2_neg', positive=True)  # |b₂|, actual b₂ = -b2_neg
...
if sol2:
    ...
    # H22 = -2a + 2b1*(2λ*²) = -2a + 4b1 * a/(2(b1 - b2_neg))
    # = -2a + 2a b1/(b1 - b2_neg)
    # = 2a [-1 + b1/(b1-b2_neg)]
    # = 2a [(-b1+b2_neg+b1)/(b1-b2_neg)]
    # = 2a b2_neg/(b1 - b2_neg)
    # This is positive when b1 > b2_neg
    print(f"Stability requires b₁ > |b₂| (i.e., b₁ > b₂_neg)")
    print(f"This is a physical constraint: b₁ must dominate over |b₂|")
...
tests.append(("b₂ < 0, n_d=4: k=1 gives minimum energy", t4))

# Test 5: Numerical check for b₂>0 → k=4 minimum
energies_pos = []
b2_p = Rational(1, 20)
for kv in range(1, 5):
...
tests.append(("b₂ > 0, n_d=4: k=4 gives minimum energy", t5))

# Test 6: b₂=0 gives exactly same energy for all k
energies_zero = []
for kv in range(1, 5):
    E = -kv * a_n**2 / (4 * (b1_n * kv))
    energies_zero.append(E)
t6 = all(e == energies_zero[0] for e in energies_zero)
tests.append(("b₂ = 0: all k give identical energy", t6))
...
t8 = t6  # Same as test 6 — b₂=0 means degenerate
tests.append(("AXM_0109 requires b₂ ≠ 0 (degenerate contradicts crystal existence)", t8))

# Test 9: Stability at k=1 for specific values
a_test, b1_test, b2_test = Rational(1, 100), Rational(1, 5), Rational(1, 10)
lam_test_sq = a_test / (2 * (b1_test - b2_test))  # b₂ actual = -b2_test
H_test = -2*a_test + 4*b1_test*lam_test_sq
t9 = H_test > 0
tests.append(("Hessian positive at k=1 minimum (b₁ > |b₂|)", t9))

# Test 10: Energy ordering matches sign
# b₂ < 0: F(1) < F(2) < F(3) < F(4)
t10 = energies_neg[0] < energies_neg[1] < energies_neg[2] < energies_neg[3]
tests.append(("b₂ < 0: strict ordering F(1) < F(2) < F(3) < F(4)", t10))
...
PROVEN (mathematical theorems):
  1. F(k) = -ka²/(4(b₁k + b₂)) for the k-eigenvalue partition on Herm(n_d)
  2. b₂ = 0 → F(k) independent of k → degenerate ground state
  3. b₂ < 0 → k=1 is global minimum (maximally broken, SU(3)×U(1))
  4. b₂ > 0 → k=n_d is global minimum (isotropic, no breaking)

DERIVED from axioms:
  5. AXM_0109 (crystal existence) requires a definite eigenvalue partition
  6. Degenerate ground state (b₂=0) → no definite partition → contradicts AXM_0109
  7. Therefore b₂ ≠ 0  [DERIVATION from AXM_0109]

CONJECTURED (physical interpretation):
  8. Crystallization creates structure (symmetry breaking)  [A-PHYSICAL]
  9. k=1 partition breaks symmetry maximally → b₂ < 0     [CONJECTURE]

GAP NARROWED:
  Before: b₂ < 0 was pure [CONJECTURE] — "inferred from AXM_0117"
  After:  b₂ ≠ 0 is [DERIVATION from AXM_0109]
          b₂ < 0 is [CONJECTURE with physical motivation]

CONFIDENCE UPGRADE:
  C2 eigenvalue selection: b₂ ≠ 0 from [CONJECTURE] → [DERIVATION]
  C2 stage 1 selection: b₂ < 0 remains [CONJECTURE]
  SU(3) emergence: [DERIVATION] given b₂ < 0 (unchanged)
""")
