...
print(f"    5 * Im_H^2 = 5 * {Im_H**2} = {expected_so10_b}")
...
print(f"    (Im_H * Im_O * n_c) / n_d^2 = {expected_ratio} = {float(expected_ratio):.4f}")
print(f"    Match: {test8}")
...
print(f"\nE6: dim = {dim_E6}")
print(f"   = C * (Im_H^2 + Im_O * Im_H * C) = {C * (Im_H**2 + Im_O * Im_H * C)}")
...
print(f"\nE8: dim = {dim_E8}")
print(f"   = O * 31 = {O * 31}")
print(f"   = n_d^2 * (C + Im_H)^2 - O = {n_d**2 * (C + Im_H)**2 - O}")
...
tests = [
    ...
    ("dim(so(10)) = 5 * Im_H^2 = 45 (GUT)", test3),
