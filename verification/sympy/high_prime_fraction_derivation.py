...
for prime, obs, n, d, desc in fractions_data:
    ...
    for a, na in [(1, "R"), (2, "C"), (3, "Im_H"), (4, "H"), (7, "Im_O"), (8, "O"), (11, "n_c")]:
        for b, nb in [(1, "R"), (2, "C"), (3, "Im_H"), (4, "H"), (7, "Im_O"), (8, "O"), (11, "n_c")]:
            if a * b == n and a <= b:
                n_forms.append(f"{na}×{nb}" if na != nb else f"{na}²" if a != 1 else na)
    ...
    for a, na in [(1, "R"), (2, "C"), (3, "Im_H"), (4, "H"), (7, "Im_O"), (8, "O"), (11, "n_c")]:
        for b, nb in [(1, "R"), (2, "C"), (3, "Im_H"), (4, "H"), (7, "Im_O"), (8, "O"), (11, "n_c")]:
            if a * b == d and a <= b:
                d_forms.append(f"{na}×{nb}" if na != nb else f"{na}²" if a != 1 else na)
...
# eta'/u = 313 × 17/12
print("\n--- eta'/u = 17/12 ---")
print(f"  17 = R² + H² = {1**2 + 4**2} = scalar-local structure")
print(f"  12 = Im_H × H = {3 * 4} = generation × local")
print(f"  Ratio = (scalar + local²) / (generation × local)")
print(f"       = (meson structure) / (quark structure)")

# Xi0/d = 181 × 14/9
print("\n--- Xi0/d = 14/9 ---")
print(f"  14 = C × Im_O = {2 * 7} = EM-color coupling")
print(f"  9 = Im_H² = {3**2} = generation²")
print(f"  Ratio = (EM × color) / generation²")
print(f"       = (baryon interactions) / (quark identity)")

# W/Xi = 139 × 7/16
print("\n--- W/Xi = 7/16 ---")
print(f"  7 = Im_O = color imaginary")
print(f"  16 = H² = {4**2} = local² OR C×O = {2*8}")
print(f"  Ratio = color / local²")
print(f"       = (strong coupling) / (spacetime structure)")

# H0 = 307 × 9/41
print("\n--- H0 = 9/41 ---")
print(f"  9 = Im_H² = {3**2} = generation²")
print(f"  41 = H² + 5² = 16 + 25 = {16 + 25} (local² + (local+scalar)²)")
print(f"  Ratio = generation² / spacetime-selfinteraction")
...
# structure_meson = R² + H² = 1 + 16 = 17 (scalar-local for singlet)
...
# structure_quark = Im_H² = 9 (generation²)
predictions.append({
    "obs": "Xi0/d",
    "scale_high": "baryon (EM-color)",
    "scale_low": "quark (generation²)",
    "pred_num": 14,  # C × Im_O
    "pred_den": 9,   # Im_H²
    "actual_num": 14,
    "actual_den": 9,
...
# structure_baryon = H² = 16 (local²) OR C×O = 16
predictions.append({
    "obs": "W/Xi",
    "scale_high": "W boson (color imaginary)",
    "scale_low": "baryon (local² or C×O)",
    "pred_num": 7,   # Im_O
    "pred_den": 16,  # H² = C×O
    "actual_num": 7,
    "actual_den": 16,
...
# H0: cosmological
# structure = generation² / spacetime interaction
predictions.append({
    "obs": "H0_CMB",
    "scale_high": "generation²",
...
1. MESON-QUARK transitions:
   Fraction = (R² + H²) / (Im_H × H) = 17/12
   Meaning: (scalar-local) / (generation-local)

2. BARYON-QUARK transitions:
   Fraction = (C × Im_O) / Im_H² = 14/9
   Meaning: (EM-color) / generation²

3. ELECTROWEAK-BARYON transitions:
   Fraction = Im_O / H² = 7/16
   Meaning: color / local²

4. COSMOLOGICAL transitions:
   Fraction = Im_H² / (H² + 5²) = 9/41
   Meaning: generation² / spacetime-selfinteraction
...
| Baryon-Quark | EM-color (14) | generation² (9) |
| EW-Baryon | color (7) | local² (16) |
| Cosmological | generation² (9) | spacetime-self (41) |
