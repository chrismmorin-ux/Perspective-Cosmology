#!/usr/bin/env python3
"""
Cyclotomic Two-Sector Hypothesis: Structural Origin of Phi_6 Denominators

KEY FINDING: Framework formulas use Phi_6(k) = k^2 - k + 1 at THREE division
algebra dimensions, corresponding to three sectors of the crystal algebra:

  Full crystal:    u(n_c = 11) -> Phi_6(11) = 111 -> EM coupling (alpha)
  Spacetime:       u(n_d = 4)  -> Phi_6(4)  = 13  -> heavy-light quark transition
  Hidden (octonionic): u(Im_O = 7)  -> Phi_6(7)  = 43  -> mass ratio corrections

The three sectors satisfy n_c = n_d + Im_O (crystal = spacetime + hidden).

Phi_6(k) = k^2 - k + 1 counts EM-active channels in u(k):
  Total generators: k^2
  EM-neutral (Cartan of SU(k)): k - 1
  EM-active: k^2 - (k-1) = k^2 - k + 1

This script tests:
1. Three-sector decomposition and Phi_6 evaluations
2. Sector assignment: which formulas use which sector
3. EM channel interpretation for all three sectors
4. Composition: Phi_6(n_c) vs Phi_6(n_d) + Phi_6(Im_O)
5. Quark hierarchy denominators as Phi_6 values
6. Primality pattern: Phi_6 prime iff argument is division algebra dim
...
def phi6(k):
    """Phi_6(k) = k^2 - k + 1, the 6th cyclotomic polynomial."""
    return k**2 - k + 1


def em_channels(k):
    """EM-active channels in u(k) = k^2 - (k-1) = Phi_6(k)."""
    return k**2 - (k - 1)
...
def test_three_sector_decomposition():
    ...
    Each sector has its own u(k) algebra and Phi_6(k) EM channel count.
    ...
    # Total generators = k^2, Cartan = k-1, EM = k^2 - k + 1
    ...
    # But Phi_6 is NOT additive: Phi_6(11) != Phi_6(4) + Phi_6(7)
    # 111 != 13 + 43 = 56
    not_additive = (phi6_crystal != phi6_spacetime + phi6_hidden)

    # Instead: n_c^2 = (n_d + Im_O)^2 = n_d^2 + 2*n_d*Im_O + Im_O^2
    # 121 = 16 + 56 + 49
...
def test_sector_assignment():
    """Map each known Phi_6-denominator formula to its sector.

    CRYSTAL sector (Phi_6(n_c) = 111):
      1/alpha = 137 + 4/111  [EM coupling -- uses full crystal]

    HIDDEN sector (Phi_6(Im_O) = 43):
      v/m_p   = 262 + 18/43  [EW/QCD mass ratio -- octonion sector]
      m_mu/m_e = 207 - 10/43  [generation mass ratio -- octonion sector]
      m_u/m_s = 1/43          [light quark ratio -- octonion sector]

    SPACETIME sector (Phi_6(n_d) = 13):
...
def test_cross_term_structure():
    """Phi_6(n_c) != Phi_6(n_d) + Phi_6(Im_O), but the DIFFERENCE is meaningful.

    Phi_6(11) = 111
    Phi_6(4) + Phi_6(7) = 13 + 43 = 56
    Difference = 111 - 56 = 55 = n_c * (n_c - 1)/2 * ... ?

    Actually: Phi_6(a+b) = Phi_6(a) + Phi_6(b) + 2ab - (a+b) + 1
    Check: Phi_6(4+7) = 13 + 43 + 56 - 11 + 1 = 102 != 111

    More precisely:
    (a+b)^2 - (a+b) + 1 = a^2 + 2ab + b^2 - a - b + 1
    a^2 - a + 1 = Phi_6(a)
    b^2 - b + 1 = Phi_6(b)
    So Phi_6(a+b) = Phi_6(a) + Phi_6(b) + 2ab - 1
...
# =============================================================================
# Test 5: Quark hierarchy denominators as Phi_6 values
# =============================================================================

def test_quark_hierarchy_phi6():
    """The quark mass hierarchy uses these denominators:

    | Ratio   | Denom | = Phi_6(k) ? | k      | Sector    |
    |---------|-------|-----------|--------|-----------|
    | y_t     | 121   | NO (= n_c^2) | --   | Full crystal (total gens) |
    | m_b/m_t | 121   | NO (= n_c^2) | --   | Full crystal (total gens) |
    | m_c/m_b | 10    | NO        | --      | Goldstone modes (n_c-1) |
    | m_s/m_c | 13    | YES       | 4=n_d  | Spacetime sector |
    | m_d/m_s | 20    | NO        | --      | Mixed? |
    | m_u/m_s | 43    | YES       | 7=Im_O | Hidden sector |

    2 of 6 denominators are Phi_6 values. The others use:
    121 = n_c^2 = dim(u(n_c)) total
    ...
    # Check which are Phi_6 values at division algebra dimensions
    ...
    # NOT a Phi_6 value at any standard dimension
...
def test_primality_at_div_alg_dims():
    """Phi_6(k) is prime exactly at the "irreducible" division algebra dimensions.

    k = 1 (R):    Phi_6 = 1   NOT prime (unit)
    k = 2 (C):    Phi_6 = 3   PRIME
    k = 3 (Im_H): Phi_6 = 7   PRIME
    k = 4 (H):    Phi_6 = 13  PRIME
    k = 7 (Im_O): Phi_6 = 43  PRIME
    k = 8 (O):    Phi_6 = 57  NOT prime (3 * 19)
    k = 11 (n_c): Phi_6 = 111 NOT prime (3 * 37)
...
def test_structural_prediction():
    ...
    - EM coupling (alpha) should use u(n_c) -> denominator 111
    ...
    alpha = EM coupling -> involves ALL charged particles -> FULL crystal [OK]
    v/m_p = EW/QCD scale -> mass generation -> HIDDEN (octonion) sector [OK]
    m_mu/m_e = generation ratio -> mass hierarchy -> HIDDEN sector [OK]
    m_u/m_s = lightest quarks -> deep non-perturbative -> HIDDEN sector [OK]
    ...
    clear_cases = 4   # alpha, v/m_p, m_mu/m_e, m_u/m_s
    ...
    # Key structural claim: alpha <-> full crystal, mass <-> octonion
...
def test_numerator_constraints():
    ...
    Pattern in hidden sector: B in {-10, 1, 18}
...
def test_extended_mass_scan():
    ...
    - Koide theta_lepton = pi * 73/99 (denominator 99 = 9*11 = Im_H^2*n_c)
    - m_p/m_e = 1836 + 11/72 (denominator 72 = 8*9 = O*Im_H^2)
    ...
    # Check: 72 = O * Im_H^2 (different from 43)
    d72_decomp = (72 == O * Im_H**2)
    # 99 = Im_H^2 * n_c (different)
    d99_decomp = (99 == Im_H**2 * n_c)

    # The proton-electron mass ratio uses O * Im_H^2 = 72
    # This is the octonion dimension * generations^2 -- different from Phi_6(7) = 43
    # So m_p/m_e does NOT use EM channel counting

    # IMPORTANT: The 43-pattern is specific to:
    # (a) EW/QCD scale ratio (v/m_p)
    # (b) Lepton generation ratio (m_mu/m_e)
...
def test_falsifiability():
    ...
    3. A formula using Phi_6(k) for k NOT a division algebra dimension
    ...
    # (except through the alpha chain, where alpha itself feeds into mass formulas)
    ...
    coupling_denoms = [111]  # alpha correction
    no_coupling_43 = (43 not in coupling_denoms)

    # Check all Phi_6 arguments are division algebra dimensions
