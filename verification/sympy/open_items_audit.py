...
print(f"  sin²θ₂₃ = {sin2_theta23_PMNS} = {float(sin2_theta23_PMNS):.4f}")
print(f"  sin²θ₁₂ = {sin2_theta12_PMNS} = {float(sin2_theta12_PMNS):.4f}")
print(f"  sin²θ₁₃ = {sin2_theta13_PMNS} = {float(sin2_theta13_PMNS):.4f}")
print()
print("CKM (quark, non-associative):")
print(f"  λ (Cabibbo) = {lambda_CKM} = {float(lambda_CKM):.4f}")
print(f"  |V_cb|      = {V_cb} = {float(V_cb):.4f}")
print()
print(f"Largest PMNS angle: sin²θ₂₃ = {float(sin2_theta23_PMNS):.3f}")
print(f"Largest CKM angle:  λ²       = {float(lambda_CKM**2):.4f}")
...
print(f"|Ω_k| after {N_efolds} e-folds ~ exp(-2×{N_efolds}) = {float(flatness_suppression):.2e}")
print(f"Current bound: |Ω_k| < 0.001 (Planck 2018)")
print(f"Framework: {float(flatness_suppression):.2e} << 0.001 ✓")
print()

# Also: Omega_m + Omega_Lambda = 1 exactly from framework
Omega_m = R(63, 200)
Omega_L = R(137, 200)
print(f"Ω_m + Ω_Λ = {Omega_m} + {Omega_L} = {Omega_m + Omega_L} (exact)")
print()
...
print("=" * 70)
print("PART 4: D11 — Neutrino Predictions (m₁=0, normal ordering)")
print("=" * 70)
...
print(f"Prediction: m₁ = 0 (rank-2 mass matrix)")
print(f"R₃₁ = Δm²₃₁/Δm²₂₁ = {R_31} = Im_H × n_c (measured: ~33.5, 1.7%)")
print(f"R₃₂ = Δm²₃₂/Δm²₂₁ = {R_32} = dim_H × dim_O (measured: ~32.5, 1.8%)")
print()
...
print(f"Σm = {m_sum*1000:.1f} meV (cosmological bound: < 120 meV)")
print()
...
tests = [
    ...
    ("H6: Ω_m + Ω_Λ = 1 exactly",
     Omega_m + Omega_L == 1),

    # D11: Neutrino predictions
    ("D11: R₃₁ = Im_H × n_c = 33",
     R_31 == 33),
    ("D11: R₃₂ = dim_H × dim_O = 32",
     R_32 == 32),
