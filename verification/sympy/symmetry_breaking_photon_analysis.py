...
KEY QUESTION: When the tilt field epsilon acquires a VEV, does the breaking
...
# For a Hermitian matrix epsilon in Herm(n), the U(n)-invariant potential at quartic
# order has TWO independent invariants:
#   I2 = Tr(epsilon^2) = Sigma lambda_i^2
#   I4 = Tr(epsilon^4) = Sigma lambda_i^4
#
# The quartic potential is:
#   V(epsilon) = -a I2 + b I2^2 + c I4
#
...
print("For V(epsilon) = -a Tr(epsilon^2) + b [Tr(epsilon^2)]^2 + c Tr(epsilon^4):")
print()
print("The eigenvalue pattern at the minimum depends on sign(c):")
print("  c > 0: Eigenvalues cluster -> epsilon* ~ I_n -> U(n) PRESERVED")
print("  c < 0: Eigenvalues spread -> epsilon* diagonal, distinct -> U(1)^n")
...
# Case 1: epsilon* proportional to identity (c > 0)
print("CASE 1: epsilon* = epsilon_0 I_n (proportional to identity)")
print("-" * 50)
print(f"  For Herm({n_d}): epsilon_d* = epsilon_0 I_{n_d}")
print(f"  For Herm({n_c}): epsilon_c* = epsilon_0 I_{n_c}")
...
# Case 2: epsilon* with ALL distinct eigenvalues (c < 0, strong)
print(f"CASE 2: epsilon* diagonal with all {n_d}+{n_c}={n_d+n_c} distinct eigenvalues")
print("-" * 50)
cartan_d = n_d
cartan_c = n_c
...
print("-" * 50)
print(f"  Crystal epsilon_c*: 4 eigenvalues = lambda_1, 7 eigenvalues = lambda_2")
print(f"  Defect epsilon_d*: all eigenvalues = lambda_0 (identity-like)")
print()
...
print(f"  Crystal unbroken dim: 4^2 + 7^2 = {4**2 + 7**2} = 65")
...
print("       to all 137 interface modes, giving alpha = 1/137.")
print()
print("ANALYSIS:")
print()
print("For the photon to couple equally to ALL 137 modes, the VEV must")
print("treat all generators symmetrically. This requires epsilon* ~ I (identity).")
print()
print("But epsilon* ~ I means [Q, epsilon*] = [Q, epsilon_0 I] = 0 for ALL generators Q.")
...
print("their non-zero commutator with epsilon*.")
print()
...
print("  The photon generator is Q_gamma = T^3_L + Y/2")
print("  This is a SPECIFIC combination -- not democratic.")
print()
print("In the framework breaking U(4)*U(11) -> SU(3)*SU(2)*U(1):")
print("  The photon is the U(1) generator that commutes with epsilon*")
...
# In GUT theories: alpha_i = g_GUT^2/(4pi) * k_i
...
print("    1/(4g^2_eff) = N_I * C/(16pi^2) * log(Lambda/mu)")
print("    Then 1/g^2 ~ N_I (but with logarithmic running)")
print("    This gives alpha_EM ~ 1/(N_I * log), not exactly 1/N_I")
print()

# Compute what induced gauge coupling would give
print("    Induced coupling estimate:")
C_loop = R(1, 12)  # Typical loop coefficient 1/(12pi^2) ~ 1/118
log_factor = 10  # log(Lambda/mu) ~ 10 for typical scales
induced_1_over_g2 = N_I * C_loop * log_factor
print(f"    1/g^2_induced ~ N_I * 1/12 * log(Lambda/mu)")
print(f"    ~ {N_I} * {C_loop} * {log_factor} = {float(induced_1_over_g2):.1f}")
print(f"    alpha_induced ~ 1/(4pi * {float(induced_1_over_g2):.1f}) ~ {float(1/(4*pi*induced_1_over_g2)):.4f}")
print(f"    Measured alpha ~ 1/137 ~ {float(R(1,137)):.6f}")
...
print("    g_4^2 = g_D^2 / V_internal")
print("    If V_internal ~ N_I and g_D is fixed, g_4^2 ~ 1/N_I")
print("    Then alpha = g_4^2/(4pi) = 1/(4pi N_I) ~ 1/1722")
print(f"    [!] Off by factor of 4pi! Gives alpha ~ {float(1/(4*pi*N_I)):.6f}")
print(f"    vs measured alpha ~ 0.007297")
print()

print("(c) VOLUME IN NATURAL UNITS (4pi absorbed)")
print("    If V_internal = 4pi * N_I (sphere of radius ~ sqrtN_I):")
print(f"    alpha = 1/(4pi) * (4pi/N_I * 4pi) = ... still doesn't give 1/N_I")
print()

print("(d) NON-STANDARD NORMALIZATION")
print("    If the framework's kinetic term is S = (N_I/16pi) integral F^2:")
print("    Then 1/(4g^2) = N_I/(16pi) -> g^2 = 4pi/N_I")
print("    alpha = g^2/(4pi) = 1/N_I = 1/137 [OK]")
print("    But this ASSUMES the factor N_I/(16pi), which is Sub-problem C")
print()
...
# For U(n) with VEV epsilon* = diag(lambda_1, ..., lambda_n):
# The mass of the gauge boson for generator T^a is:
#   m_a^2 ~ Tr([T^a, epsilon*]+[T^a, epsilon*]) = Tr([T^a, epsilon*]^2)
#
# For diagonal epsilon* and off-diagonal generator E_{ij}:
#   [E_{ij}, epsilon*] = (lambda_j - lambda_i) E_{ij}
#   m_{ij}^2 ~ (lambda_i - lambda_j)^2
#
# For diagonal T^a (Cartan generator):
#   [T^a, epsilon*] = 0
#   m_a = 0 (always massless!)

print("For VEV epsilon* = diag(lambda_1, lambda_2, ..., lambda_n):")
print()
print("Gauge boson masses:")
print("  Off-diagonal E_{ij}: m^2 ~ (lambda_i - lambda_j)^2")
print("  Diagonal T^a:        m^2 = 0 (always massless)")
print()
...
print("For crystal epsilon_c* = diag(lambda_1*4, lambda_2*7):")
print("  Within 4-block: (lambda_1 - lambda_1)^2 = 0 -> massless (U(4) generators)")
print("  Within 7-block: (lambda_2 - lambda_2)^2 = 0 -> massless (U(7) generators)")
print("  Cross 4<->7:     (lambda_1 - lambda_2)^2 > 0 -> MASSIVE (56 generators)")
print()
...
print()
print("  U(7) -> G_2 -> SU(3): dim 49 -> 14 -> 8")
...
print("  alpha_i(M_GUT) = alpha_GUT for all gauge factors (universal)")
print()
...
print("  - The tree-level coupling is alpha_GUT (a free parameter)")
print("  - The number of generators affects RUNNING only")
print("  - At low energy: alpha_EM != 1/N_generators")
print()
...
print()
print("  1. Democratic coupling requires epsilon* ~ I (identity VEV)")
...
print("  [OK] The breaking pattern U(4)*U(11) -> SU(3)*SU(2)*U(1) can be")
print("    made explicit (strengthens SM gauge group derivation)")
print("  [OK] The photon can be identified as a specific generator")
print("  [OK] The number of massive/massless modes can be counted")
print("  [X] The coupling alpha = 1/137 cannot be derived this way")
print()
print("IMPLICATION FOR STEP 5:")
print("  The 'democratic' interpretation of alpha = 1/N_I is INCOMPATIBLE")
...
print("  (c) alpha = 1/137 is a coincidence")
print()
...
tests = [
    ...
    ("Identity VEV: stabilizer = full U(n), dim = n^2",
    ...
    ("Democratic epsilon* ~ I -> [Q, epsilon*] = 0 -> no breaking (structural)",
     True),  # Proven above

    ("Non-identity epsilon* -> some [Q, epsilon*] != 0 -> unequal coupling (structural)",
     True),  # Proven above

    # KK estimate
    ("KK mechanism: alpha = 1/(4pi N_I) ~ 5.8e-4 (off by 4pi from 1/137)",
     abs(1/(4 * float(pi) * N_I) - 1/137) > 0.005),

    # Induced gauge estimate gives logarithmic, not linear
    ("Induced gauge: 1/g^2 ~ N_I * log -- logarithmic dependence",
     True),  # Structural argument

    # The only mechanism that gives 1/N_I exactly is if
    # the kinetic term coefficient IS N_I/(16pi)
    ("Non-standard normalization: S = (N_I/16pi)F^2 gives alpha = 1/N_I",
     True),  # This is Sub-problem C, assumed not derived

    # Framework numbers
    ("137 - 12 = 125 = 5^3 (number of broken generators)",
     N_I - 12 == 125),
...
print()
print("The coupling alpha = 1/137 CANNOT be derived from symmetry breaking")
