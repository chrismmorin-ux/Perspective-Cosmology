...
2. Up-type (u,c,t): A^2 = 34/11 = (Im(H)*n_c + R)/n_c with 0.05% error
3. Down-type (d,s,b): A^2 = 19/8 = (C*O + Im(H))/O with 0.52% error
4. Heavy (c,b,t): A^2 = 127/63 = 2 + 1/(Im(O)*Im(H)^2) with 0.004% error!
...
def A_squared_from_Q(Q):
    """Invert Koide formula: Q = (1 + A^2/2)/3 -> A^2 = 6Q - 2"""
    return 6*Q - 2
...
print(f"""
| Triplet     | Q        | Dev from 2/3 | A^2       | A^2 - 2    |
|-------------|----------|--------------|----------|-----------|
...
print("=" * 70)
print("DIVISION ALGEBRA FORMULAS FOR A^2")
print("=" * 70)

# LEPTONS: A^2 = 2 = dim(C) [KNOWN]
print()
print("LEPTONS: A^2 = dim(C) = 2")
print("-" * 50)
A2_lep_pred = dim_C
Q_lep_pred = Q_from_A_squared(A2_lep_pred)
print(f"  Formula: A^2 = dim(C) = {dim_C}")
print(f"  Predicted A^2 = {A2_lep_pred}")
print(f"  Observed A^2  = {A2_lep:.6f}")
print(f"  Error: {abs(A2_lep_pred - A2_lep)/A2_lep*100:.4f}%")

# UP-TYPE: A^2 = 34/11 = (Im(H)*n_c + R)/n_c
print()
print("UP-TYPE (u,c,t): A^2 = (Im(H)*n_c + R)/n_c = 34/11")
print("-" * 50)
A2_up_pred = (Im_H * n_c + dim_R) / n_c
frac_up = Fraction(Im_H * n_c + dim_R, n_c)
Q_up_pred = Q_from_A_squared(A2_up_pred)
print(f"  Formula: A^2 = ({Im_H}*{n_c} + {dim_R})/{n_c}")
print(f"         = ({Im_H * n_c} + {dim_R})/{n_c}")
print(f"         = {Im_H * n_c + dim_R}/{n_c}")
print(f"         = {frac_up}")
print(f"  Predicted A^2 = {A2_up_pred:.6f}")
print(f"  Observed A^2  = {A2_up:.6f}")
...
print()
print("DOWN-TYPE (d,s,b): A^2 = (C*O + Im(H))/O = 19/8")
print("-" * 50)
A2_down_pred = (dim_C * dim_O + Im_H) / dim_O
frac_down = Fraction(dim_C * dim_O + Im_H, dim_O)
Q_down_pred = Q_from_A_squared(A2_down_pred)
print(f"  Formula: A^2 = ({dim_C}*{dim_O} + {Im_H})/{dim_O}")
print(f"         = ({dim_C * dim_O} + {Im_H})/{dim_O}")
print(f"         = {dim_C * dim_O + Im_H}/{dim_O}")
print(f"         = {frac_down}")
print(f"  Predicted A^2 = {A2_down_pred:.6f}")
print(f"  Observed A^2  = {A2_down:.6f}")
...
print()
print("HEAVY (c,b,t): A^2 = 2 + 1/(Im(O)*Im(H)^2) = 127/63")
print("-" * 50)
...
print(f"  Formula: A^2 = 2 + 1/({Im_O}*{Im_H}^2)")
print(f"         = 2 + 1/({Im_O}*{Im_H**2})")
print(f"         = 2 + 1/{denom_heavy}")
print(f"         = {frac_heavy}")
print(f"  Predicted A^2 = {A2_heavy_pred:.6f}")
print(f"  Observed A^2  = {A2_heavy:.6f}")
...
print()
print("=" * 70)
print("SUMMARY: QUARK KOIDE A^2 FROM DIVISION ALGEBRAS")
print("=" * 70)

print(f"""
| Triplet  | Formula                    | A^2 exact | A^2 obs   | Error   |
|----------|----------------------------|----------|----------|---------|
...
| Heavy (c,b,t)| 2 + 1/(Im(O)*Im(H)^2)    | 127/63   | {A2_heavy:.6f} | {abs(127/63-A2_heavy)/A2_heavy*100:.4f}% |
""")
...
tests = [
    ("Lepton A^2 = 2 (exact)", abs(A2_lep - 2) < 0.001),
    ("Up-type A^2 = 34/11 (0.1%)", abs(A2_up - 34/11)/A2_up < 0.001),
    ("Down-type A^2 = 19/8 (1%)", abs(A2_down - 19/8)/A2_down < 0.01),
    ("Heavy A^2 = 127/63 (0.01%)", abs(A2_heavy - 127/63)/A2_heavy < 0.0001),
